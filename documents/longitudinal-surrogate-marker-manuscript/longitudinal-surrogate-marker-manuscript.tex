...
\begin{enumerate}
    ...
    \item $\forall \bx \in \suppxo, 0 < C\rho(h) \leq P\subxo\{\bX \in B(\bx, h)\} \leq C\rho(h) < \infty$. %%% I think this can be replaced by positive support?
    ...
    \] %%% don't need this if we don't need the second one.
    %\item For $n$ large enough, $\frac{(\log n)^2}{n\rho(h)} < \psi_{\suppxo}(\frac{\log n}{n}) < \frac{n\rho(h)}{\log n}$. %%% this should be a direct consequence of the fractal property
    %\item Kolmogorov's $\epsilon$-entropy of $\suppxo$ satisfies, for some $\beta > 1$ 
    %\[
    %    \sum_{n=1}^\infty \exp \left\{(1-\beta)\psi_\suppxo\left(\frac{\log n}{n}\right) \right\} < %\infty
    %\] %% this one's not need, as it's guranteed by the semi-metric space
