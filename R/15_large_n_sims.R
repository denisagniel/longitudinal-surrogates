...
saveRDS(sim_res, 
        here('results/large_n_sims.rds'))
final_res <- map(1:nrow(sim_parameters), function(i) {
...
final_sims <- final_res %>% bind_rows
write_csv(final_sims,
          here('results/large_n_sims.csv'))
